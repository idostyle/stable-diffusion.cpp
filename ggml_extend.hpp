    ...
    void init_params(struct ggml_context* ctx, std::map<std::string, enum ggml_type>& tensor_types, const std::string prefix = "") {
        if (tensor_types.find(prefix + "A") != tensor_types.end()) {
            ...
            if (prefix.find("proj") != std::string::npos) {
                rank = 1024;
            } else if (prefix.find("qkv") != std::string::npos) {
                rank = 1280;
            } else if (prefix.find("mlp.2") != std::string::npos) {
                rank = 1408;
            } else if (prefix.find("mlp.0") != std::string::npos) {
                rank = 1344;
            } else if (prefix.find("modulation.lin") != std::string::npos) {
                rank = 1280;
            } else if (prefix.find("_mod.lin.") != std::string::npos) {
                rank = 1344;
            } else if (prefix.find("linear1") != std::string::npos) {
                rank = 1600;
            } else if (prefix.find("linear2") != std::string::npos) {
                rank = 1600;
            } else if (prefix.find("time_in.in_layer") != std::string::npos) {
                rank = 128;
                ...
                rank = 1088;
            } else if (prefix.find("guidance_in.in_layer") != std::string::npos) {
                rank = 192;
            } else if (prefix.find("guidance_in.out_layer") != std::string::npos) {
                rank = 1280;
            }

            params["A"] = ggml_new_tensor_2d(ctx, wtype, in_features, rank);
            params["B"] = ggml_new_tensor_2d(ctx, wtype, rank, out_features);
        } else {
